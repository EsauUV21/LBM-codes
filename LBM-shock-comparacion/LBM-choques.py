...
def paso(rho, u, E, T, p, w_f, w_h, f, h):
    ...
    f_l_aux = jnp.zeros((Q, N_y, N_x + 4)).at[..., 2:(N_x+2)].set(f_1);
    f_l_aux = f_l_aux.at[..., :2].set(f_1[:, :, [0]]);
    f_l_aux = f_l_aux.at[..., -2:].set(f_1[:, :, [-1]]);

    h_l_aux = jnp.zeros((Q, N_y, N_x + 4)).at[..., 2:(N_x+2)].set(h_1);
    h_l_aux = h_l_aux.at[..., :2].set(h_1[..., [0]]);
    h_l_aux = h_l_aux.at[..., -2:].set(h_1[..., [-1]]);

    point_grad_f = calc_deriv(f_l_aux, 2)[..., 2:(N_x+2)] + calc_deriv(f_1, 1);
    point_grad_h = calc_deriv(h_l_aux, 2)[..., 2:(N_x+2)] + calc_deriv(h_1, 1);
...
def prueba():
    ...
    rho = rho.at[alta].set(1.0 * rho0);
    rho = rho.at[baja].set(1.0 * rho0);

    p = p.at[alta].set(p0 * 10.0);
    p = p.at[baja].set(p0 * 0.00001);
    T = (1 / R) * p / rho;
    _05_u_2 = 0.5 * jnp.sum(u * u, axis = 0);
    E = 0.5 * b * R * T + _05_u_2;
